window.onload = function () {
	...
	var E="初十廿卅";
	var V={"0101":"*1元旦节","0214":"情人节","0305":"学雷锋纪念日","0308":"妇女节","0312":"植树节","0315":"消费者权益日","0401":"愚人节","0501":"*1劳动节","0504":"青年节","0601":"国际儿童节","0701":"中国共产党诞辰","0801":"建军节","0910":"中国教师节","1001":"*3国庆节","1224":"平安夜","1225":"圣诞节"};
	var T={"0101":"*2春节","0115":"元宵节","0505":"*1端午节","0815":"*1中秋节","0909":"重阳节","1208":"腊八节","0100":"除夕"};
	function U(Y) {
		function c(j, i) {
	...
	var W = (function () {
		...
		function Z() {
			...
			b(g);
			Q.init(g);
			var year = g.solarYear;
		    var month = g.solarMonth < 10 ? "0" + g.solarMonth : g.solarMonth;
		    var date = g.solarDate < 10 ? "0" + g.solarDate : g.solarDate;
		    var kqrq = year+""+month+""+date;
			$.ajax({  
	          type : "post",  
	          url : "kqgl.do?method=gjrqLoad",  
	          data : "kqrq=" + kqrq,  
		...
		function d(k, g) {
			...
			C.onchange = f; //改变年触发事件
			X.onchange = f; //改变月触发事件
		}
		function e(g) {
	...
	var N = (function () {
		function C() {
			var Z = Q.getJson();
			var c = Z.dateArray;
			M("cm").style.height = Z.lines * 68 + 2 + "px";
			M("cm").innerHTML = "";
			var i=-1;//kqfList数组下标。 这样减少循环
			for (var a = 0; a < c.length; a++) {
				...
				var X = R("DIV");
				//if (c[a].isToday) { //设置今天的背景色
				//	X.style.border = "1px solid #a5b9da";
				//	X.style.background = "#c1d9ff";
				//}
				
				var year = c[a].solarYear;
			    var month = c[a].solarMonth < 10 ? "0" + c[a].solarMonth : c[a].solarMonth;
			    var date = c[a].solarDate < 10 ? "0" + c[a].solarDate : c[a].solarDate;
			    var kqrq = year+""+month+""+date;
			    if(kqfList.length>0){
			    	/*if(kqfList[i].xgbz==0 && kqfList[i].kqrq==kqrq){//修改标志为0 不可以修改
			    		if(kqfList[i].zt==0 && kqfList[i].kqrq==kqrq){
							X.style.backgroundImage = 'url(../kqgl/img/gqwt.png)';//过期未填
						}else if(kqfList[i].zt==1 && kqfList[i].kqrq==kqrq){
							X.style.backgroundImage = 'url(../kqgl/img/gqyt.png)';//过期已填
						}else if(kqfList[i].zt==2 && kqfList[i].kqrq==kqrq){
							X.style.backgroundImage = 'url(../kqgl/img/wgqwt.png)';//未过期未填
						}else if(kqfList[i].zt==3 && kqfList[i].kqrq==kqrq){
							X.style.backgroundImage = 'url(../kqgl/img/wgqyt.png)';//未过期已填
						}else if(kqfList[i].zt==4 && kqfList[i].kqrq==kqrq){
							X.style.backgroundImage = 'url(../kqgl/img/swdq.png)'; //尚未到期 
						}
			    	}else if(kqfList[i].xgbz==1 && kqfList[i].kqrq==kqrq){ //修改标志为1 都可以修改
			    		if(kqfList[i].zt==0 && kqfList[i].kqrq==kqrq){
							X.style.backgroundImage = 'url(../kqgl/img/gqwt.png)';//过期未填
						}else if(kqfList[i].zt==1 && kqfList[i].kqrq==kqrq){
							X =R("A");//过期已填
							X.href="#";
							X.value=kqrq;
							X.style.backgroundImage = 'url(../kqgl/img/gqyt.png)';
						}else if(kqfList[i].zt==2 && kqfList[i].kqrq==kqrq){
							X =R("A");//未过期未填
							X.href="#";
							X.value=kqrq;
							X.style.backgroundImage = 'url(../kqgl/img/wgqwt.png)';
						}else if(kqfList[i].zt==3 && kqfList[i].kqrq==kqrq){
							X =R("A"); //未过期已填
							X.href="#";
							X.value=kqrq;
							X.style.backgroundImage = 'url(../kqgl/img/wgqyt.png)';
						}else if(kqfList[i].zt==4 && kqfList[i].kqrq==kqrq){
							X.style.backgroundImage = 'url(../kqgl/img/swdq.png)'; //尚未到期 
						}
			    	}*/
			    ...
			    if(kqfList.length>0){
			    	if(kqfList[i].xgbz==1 && kqfList[i].zt==2 && kqfList[i].kqrq==kqrq){
						X.className = "cellwgqwt";
					}else if(kqfList[i].xgbz==1 && kqfList[i].zt==3 && kqfList[i].kqrq==kqrq){
						X.className = "cellwgqyt";
					}else if(kqfList[i].xgbz==1 && kqfList[i].zt==1 && kqfList[i].kqrq==kqrq){
						X.className = "cellgqyt";
					}else if(kqfList[i].xgbz==0 && kqfList[i].zt==2 && kqfList[i].kqrq==kqrq){
						X.className = "cellwgqwt1";
					}else if(kqfList[i].xgbz==0 && kqfList[i].zt==3 && kqfList[i].kqrq==kqrq){
						X.className = "cellwgqyt1";
					}else if(kqfList[i].xgbz==0 && kqfList[i].zt==1 && kqfList[i].kqrq==kqrq){
						X.className = "cellgqyt1";
					}else{
						X.className = "cell";
					}
			    }else{
				...
				X.style.left = (a % 7) * 90 + "px";
				X.style.top = Math.floor(a / 7) * 68 + 2 + "px";
			...
			G.id = "fd";
			M("cm").appendChild(G);
			$(".cellwgqwt").click(function(){
				var dd=$(this).val().toString().substring(0,4)+"/"+$(this).val().toString().substring(4,6)+"/"+$(this).val().toString().substring(6,8);
				var day = new Date(Date.parse(dd));
				if(day.getDay()==0 || day.getDay()==6){
					sfzm = 'Y'; //是否为周末
				}else{
					sfzm = 'N';
				}
				window.location.href="kqgl.do?method=loadKqjl&kqrq="+$(this).val()+"&sfkxg=Y"+"&sfzm="+sfzm;
			});
			$(".cellwgqyt").click(function(){
				var dd=$(this).val().toString().substring(0,4)+"/"+$(this).val().toString().substring(4,6)+"/"+$(this).val().toString().substring(6,8);
				var day = new Date(Date.parse(dd));
				if(day.getDay()==0 || day.getDay()==6){
					sfzm = 'Y';
				...
				window.location.href="kqgl.do?method=loadYtKqjl&kqrq="+$(this).val()+"&sfkxg=Y"+"&sfzm="+sfzm;
			});
			$(".cellgqyt").click(function(){
				var dd=$(this).val().toString().substring(0,4)+"/"+$(this).val().toString().substring(4,6)+"/"+$(this).val().toString().substring(6,8);
				var day = new Date(Date.parse(dd));
				if(day.getDay()==0 || day.getDay()==6){
					sfzm = 'Y';
			...
			$(".cellwgqwt1").click(function(){
				...
				window.location.href="kqgl.do?method=loadKqjl&kqrq="+$(this).val()+"&sfkxg=N"+"&sfzm="+sfzm;
			});
			$(".cellwgqyt1").click(function(){
				var dd=$(this).val().toString().substring(0,4)+"/"+$(this).val().toString().substring(4,6)+"/"+$(this).val().toString().substring(6,8);
				var day = new Date(Date.parse(dd));
				if(day.getDay()==0 || day.getDay()==6){
					sfzm = 'Y';
				...
				window.location.href="kqgl.do?method=loadYtKqjl&kqrq="+$(this).val()+"&sfkxg=N"+"&sfzm="+sfzm;
			});
			$(".cellgqyt1").click(function(){
				var dd=$(this).val().toString().substring(0,4)+"/"+$(this).val().toString().substring(4,6)+"/"+$(this).val().toString().substring(6,8);
				var day = new Date(Date.parse(dd));
				if(day.getDay()==0 || day.getDay()==6){
					sfzm = 'Y';
	...
	/*var F = (function () {
		...
		function G(b) {
			...
			var Z = b.cell;
			var c = "#{solarYear}\xa0\u5e74\xa0#{solarMonth}\xa0\u6708\xa0#{solarDate}\xa0\u65e5\xa0#{solarWeekDayInChinese}";
			c += "<br><b>\u519c\u5386\xa0#{lunarMonthInChinese}\u6708#{lunarDateInChinese}</b>";
			c += "<br>#{ganzhiYear}\u5e74\xa0#{ganzhiMonth}\u6708\xa0#{ganzhiDate}\u65e5";
	...
	//点击未填写但在可以填写考勤记录范围内的日期，跳转到填写考勤记录页面
	//$(".cellwgqwt").click(function(){
	//	window.location.href="kqgl.do?method=loadKqjl&kqrq="+$(this).val();
	//});
};
